...
const StyledObject = styled.span`{
  &.t-main {
    ...
    .lb-box .lb-li { ${cs.min.height(200)} }
...
const samplecode = (value, classname = '') => `<Combobox className={"${classname}"} ${value} />`;
...
export const object = () => {
  ...
  return (
    <StyledObject className={"t-main"}>
      <Linebox title={"checkbox"} id={"f0001"} desc={"Knobs 옵션을 통해 미리보기가 가능합니다."} top={option.top} sample={samplecode('label={"label"} list={list} radio={true}', '')}>
        <Combobox className={"primary"} label={"combo"} list={list} onClick={onClick} onChange={onChange} frameid={"f0001"} />
      </Linebox>
