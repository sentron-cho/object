...
export default {
  ...
  component: Sidemenu, // 어떤 컴포넌트를 문서화 할지 명시
  decorators: [withKnobs], // 애드온 적용
};

const samplecode = (value, classname = '') => `<Sidemenu  className={"${classname}"} ${value} />`;
export const sobject = () => {
  ...
  const bg = options('background',
    ...
    '', { display: 'inline-radio' }, 'Other');
  const title = text('title', 'Sidemenu');
  ...
  return (
    <StyledObject className={"t-main"}>
      <Linebox title={"Sidemenu"} className={""} id={"f0001"} desc={"Knobs 옵션을 통해 미리보기가 가능합니다."} top={option.top}
        sample={samplecode('title={"Sidemenu"} onClick={onClick} onChange={onChange}', 'primary')} box={true}>
      </Linebox>
