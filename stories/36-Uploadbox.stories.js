...
export default {
  title: 'object|Uploadbox', // 스토리북에서 보여질 그룹과 경로를 명시
  component: Uploadbox, // 어떤 컴포넌트를 문서화 할지 명시
...
export const sobject = () => {
  ...
  const bg = options('background',
    ...
    '', { display: 'inline-radio' }, 'Other');
  const title = text('title', 'Uploadbox');
  ...
  return (
    <StyledObject className={"t-main"}>
      <Linebox title={"Uploadbox"} className={""} id={"f0001"} desc={"Knobs 옵션을 통해 미리보기가 가능합니다."} top={option.top}
        sample={samplecode('title={"Uploadbox"} onClick={onClick} onChange={onChange}', 'primary')} box={true}>
      </Linebox>
